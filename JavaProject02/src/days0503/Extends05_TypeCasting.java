...
public class Extends05_TypeCasting {
	...
	public static void main(String[] args) {
		...
		super3.abc();  // SubC 의 오버라이딩 된 메서드가 실행
		...
		// 객체간 TypeCasting #4
		// 자식클래스의 참조변수 <- 부모클래스의 참조변수값 : 이동작은 위에서 봤듯이 안되는 동작이지만
		// 만약  부모클래스 참조변값이 저장하고 있는 참조값이 자식클래스 인스턴스의 참조값이라면   이건 가능하게 됩니다
		...
		// 부모참조변수에 자식인스턴스가 저장된 경우 // 부모가 물려준 멤버 변수 이외에는 접근이 안됩니다
		SuperC super6 = new SubC();
		// super6.subNum = 10;  // X
		
		// 당장 현재 super6에 저장된  new Subc() 의 subNum 을 사용하고자 한다면
		// 자식클래스 참조변수에 옮겨 저장하여 사용합니다.
		// 그러려면  #4 에서 언급한 타입캐스팅이 필요합니다
		...
		// 기본자료의 형변환
		short s = 10;
		int i = s;  // 작은 용량의 자료를 큰용량의 변수에 이동.
		short k = (short)i;   // 작은 용량의 변수에 큰용량의 자료를 대입.
