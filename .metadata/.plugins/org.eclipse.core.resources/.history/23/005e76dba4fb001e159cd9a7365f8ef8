...
--  not null : null 을 허용하지 않는 제약. 해당 필드가 빈칸채로는 레코드가 insert 되지 않습니다.
...
-- 6. 필드명 뒤에 DATAYPE은 반드시 지정하고 []안에 내용은 해당내용이 있을 때 작성하며 생략 가능
...
-- 13. 컬럼(필드)과 컬럼(필드)의 구분은 콤마로 하되, 마지막 컬럼이나 constaint 를 작성후엔 콤마를 찍지 않음
...
--  예를 들면 booklist 에 존재 하지 않은 도서의 번호가 rentlist의 빌려간 도서의 번호로 등록되지 못하게 하는것을 말합니다
...
CREATE TABLE rentlist(
	idx number(3) ,     -- 대여기록번호
	rent_date date default sysdate ,   -- 대여날짜 ( sysdate를 default로 주게되면 설정하지않아도 오늘날짜로 올라감)
...
-- - 단 NULL은 고유키 제약의 대상이 아님으로, NULL값을 가진 행이 여러개가 UNIQUE KEY
--   제약에 위반하지는 않습니다
-- NOT NULL
-- - 비어있는 상태, 아무것도 없는 상태를 허용하지 않습니다 - 입력필수
-- CHECK
-- - 입력할 수 있는 값의 범위를 제한한다. CHECK 제약으로는 TRUE or FALSE로 평가할 수 있는 노리식을 지정합니다
-- FOREIGN KEY
