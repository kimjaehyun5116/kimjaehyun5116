...
.container{width:560px; margin:0 auto; border:1px solid gray; display:flex; justify-content: center; align-items:center; flex-direction: column;}
...
<body> 
<%
  	Calendar sDay = Calendar.getInstance(); // 오늘 날짜 객체 생
	int sYear = sDay.get( Calendar.YEAR); // 년도 추출
	int sMonth = sDay.get( Calendar.MONTH); // 월 추출
	
	// 최초 실행 request.getParameter("sYear") 는 null 
	// 이전달이나 다음달 버튼을 클릭하고 이동온 경우는 null 도 아니고 그값으로 sYear 와 sMonth 를 교체해줘야 합니다.
...
<div class="container">
	...
	<%
		...
		int k;
		for(k=1; k<start_week; k++)		// 첫칸부터 1일자 요일 전까지 빈칸으로 
